...
os.system('tail -n 1 *.cel > lastcel')

boxhandle = open('lastcel','r')
boxline = boxhandle.readlines()
boxhandle.close()
boxsplit = boxline[0].split()
boxoutput = boxsplit[-3] + "\t" + boxsplit[-2] + "\t" + boxsplit[-1] + "\n"
...
for line in InptLines[:-len(xlines)-len(vlines)-4]:
	OutptHandle.write(line)
OutptHandle.write(boxoutput)
for line in InptLines[-len(xlines)-len(vlines)-3:-len(xlines)-len(vlines)]:
	OutptHandle.write(line)
for line in xlines:
...
os.system('rm '+nameprefix+'.cel')
os.system('rm endx')
os.system('rm lastcel')
